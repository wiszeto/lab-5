...
.text
init: li x20, 0x1100C00C   #SSEG register
...
reset:       addi x23, x23, -9
             lb x19, 0(x23)
	     sb x19, 0(x20)
             j fade


switch_delay: addi x30, x10, 0

s_delay: addi x10, x10, -1          #decrement the delay on
          bne  x10, x0, on_delay    #delay by a0 after turn on
          
          add  x10, x10, x30          #reset the delay until loop is done
          
          sw x0, 0(x20)             #turn off led

s2_delay: addi x11, x11, -1         #decrement the delay off
...
repeat: sw  x19, 0(x20) #turn on led
